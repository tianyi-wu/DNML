...
from sklearn.utils import (check_random_state, check_array, gen_batches, gen_even_slices, _get_n_jobs)
from sklearn.utils.extmath import logsumexp
from sklearn.utils.validation import NotFittedError
from sklearn.utils.validation import check_non_negative

from _VB_lda import (mean_change, _dirichlet_expectation_1d,
...
def update_dir_prior(prior, N, logphat, rho):
    ...
    **Huang: Maximum Likelihood Estimation of Dirichlet Distribution Parameters.**
    ...
    if np.all(rho * dprior + prior > 1e-8):
        prior += rho * dprior
    # else:
    #     print("updated prior not positive")
...
def _update_doc_distribution(X, exp_topic_word_distr, doc_topic_prior,
    ...
    exp_doc_topic = np.exp(_dirichlet_expectation_2d(doc_topic_distr)) + EPS
    ...
    for idx_d in xrange(n_samples):
        ...
        for _ in xrange(0, max_iters):
            ...
            norm_phi = np.dot(exp_doc_topic_d, exp_topic_word_d) + 10 * EPS
...
def update_topic_word_weight(learner, X):
    doc_topic_distr = learner.transform(X)
    learner.topic_weight, learner.topic_word_weight = calculate_topic_word_weight(X, doc_topic_distr,
                                                                                  learner.components_)


def calculate_topic_word_weight(X, doc_topic_distr, exp_topic_word_distr):
    exp_doc_topic = np.exp(_dirichlet_expectation_2d(doc_topic_distr))
    return _calculate_topic_word_weight(X, exp_doc_topic, exp_topic_word_distr)


def _calculate_topic_word_weight(X, exp_doc_topic, exp_topic_word_distr):
    is_sparse_x = sp.issparse(X)
    n_samples, n_features = X.shape
    n_topics = exp_topic_word_distr.shape[0]
    ...
    topic_weight = np.zeros(n_topics)
    word_topic_weight = np.zeros((n_features, n_topics))
    ...
    for dd in range(n_samples):
        ...
        exp_doc_topic_d = exp_doc_topic[dd, :]
        exp_topic_word_d = exp_topic_word_distr[:, ids]
        norm_phi = np.dot(exp_doc_topic_d, exp_topic_word_d) + EPS

        for n, num in enumerate(cnts):
            q_dn = exp_doc_topic_d * exp_topic_word_d[:, n] / norm_phi[n]
            word_topic_weight[ids[n]] += q_dn
            topic_weight += q_dn

    return topic_weight, word_topic_weight.T


def check_normalized(distr, axis=1):
    distr_sum = distr.sum(axis=axis)
    if np.any(np.abs(distr_sum - 1.0) > 1e-5):
        return distr / distr_sum[:, np.newaxis]
    else:
        return distr
...
def log_likelihood(X, doc_topic_distr, topic_word_distr):
    ...
    for dd in range(n_samples):
        if is_sparse_x:
            ...
            ids = np.nonzero(X[dd, :])[0]
            cnts = X[dd, ids]
        log_word_d = logsumexp((log_doc_topic_distr[dd, :])[:, np.newaxis] + log_topic_word_distr[:, ids], axis=0)
        log_lik += np.sum(log_word_d * cnts)
    return log_lik


class OnlineLatentDirichletAllocation(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_topics=10, doc_topic_prior=None,
                 ...
                 parameter_forgetting_factor=0.01,
                 partial_max_iter=10, max_iter=50,
                 partial_evaluate_every=5, evaluate_every=10,
                 # learning_decay=.7, learning_offset=10.,
                 batch_size=128,
                 # total_samples=1e6,
                 perp_tol=1e-1, mean_change_tol=1e-3, max_doc_update_iter=100,
                 max_hyper_update_iter=5,
                 n_jobs=1, verbose=0, random_state=None):
        self.n_topics = n_topics
        self.doc_topic_prior = doc_topic_prior
        self.topic_word_prior = topic_word_prior
        self.parameter_forgetting_factor = parameter_forgetting_factor
        self.learning_method = learning_method
        # self.learning_decay = learning_decay
        # self.learning_offset = learning_offset
        self.max_iter = max_iter
        self.partial_max_iter = partial_max_iter
        self.batch_size = batch_size
        self.evaluate_every = evaluate_every
        self.partial_evaluate_every = partial_evaluate_every
        # self.total_samples = total_samples
        self.perp_tol = perp_tol
        self.mean_change_tol = mean_change_tol
        self.max_doc_update_iter = max_doc_update_iter
        self.max_hyper_update_iter = max_hyper_update_iter
        self.n_jobs = n_jobs
    ...
    def update_alpha(self, gammat, rho):
        ...
        self.doc_topic_prior_ = update_dir_prior(self.doc_topic_prior_, N, logphat, rho)

        return self.doc_topic_prior_
    ...
    def update_eta(self, lambdat, rho):
        ...
        """
        if self.topic_word_prior_.shape[1] != 1:
        ...
        self.topic_word_prior_ = update_dir_prior(self.topic_word_prior_, N, logphat, rho)

        return self.topic_word_prior_
    ...
    def _init_latent_vars(self, n_sample, n_features):
        ...
        if self.doc_topic_prior is None:
            self.doc_topic_prior_single = 1.0 / n_sample
            self.doc_topic_prior_ = np.asarray([self.doc_topic_prior_single for i in xrange(self.n_topics)])
        else:
            self.doc_topic_prior_ = self.doc_topic_prior

        if self.topic_word_prior is None:
            self.topic_word_prior_single = 1.0 / n_features
            self.topic_word_prior_ = np.asarray([self.topic_word_prior_single for i in xrange(self.n_topics)]).reshape(
                (self.n_topics, 1))
        else:
            self.topic_word_prior_ = self.topic_word_prior.reshape((self.n_topics, 1))
    ...
    def _em_step(self, X, total_samples, batch_update, parallel=None):
        ...
        def rho():
            return pow(offset + pass_ + (self.n_batch_iter_ / X.shape[0]), -decay)

        # E-step
        doc_topic_distr, suff_stats = self._e_step(X, cal_sstats=True, random_init=True,
                                                   parallel=parallel)
        for i in range(self.max_hyper_update_iter):
            self.update_alpha(doc_topic_distr, rho())
        ...
        if batch_update:
            ...
            #                   -self.learning_decay)
            weight = self.parameter_forgetting_factor
        ...
        for i in range(self.max_hyper_update_iter):
            self.update_eta(self.components_, rho())
    ...
    def _check_non_neg_array(self, X, whom):
        """check _X format
        check _X format and make sure no negative value in _X.
        Parameters
        ----------
            ...
            for i in xrange(max_iter):
                ...
                if evaluate_every > 0 and (i + 1) % evaluate_every == 0:
                    doc_topic_distr, _ = self._e_step(X, cal_sstats=False,
                                                      random_init=False,
                                                      parallel=parallel)
                    bound = self.perplexity(X, doc_topic_distr,
                                            sub_sampling=False)
                    if self.verbose:
    ...
    def fit(self, X, y=None):
        """Learn model for the data _X with variational Bayes method.
    ...
    def _approx_bound(self, X, doc_topic_distr, sub_sampling):
        ...
        documents passed in as _X. Since log-likelihood of each word cannot
        ...
        def _loglikelihood(prior, distr, dirichlet_distr, size):
            ...
            score += np.sum(gammaln(np.sum(prior)) - gammaln(np.sum(distr, 1)))
            return score
    ...
    def score(self, X, y=None):
        ...
        doc_topic_distr = self.transform(X)
        self.components_ = check_normalized(self.components_)
        # score = self._approx_bound(_X, doc_topic_distr, sub_sampling=False)
        score = log_likelihood(X, doc_topic_distr, self.components_)
        return score

    def perplexity(self, X, doc_topic_distr=None, sub_sampling=False):
        """Calculate approximate perplexity for data _X.
        ...
        doc_topic_distr : None or array, shape=(n_samples, n_topics)
            ...
            If it is None, it will be generated by applying transform on _X.
        Returns
        -------
        score : float
            Perplexity score.
        ...
        if doc_topic_distr is None:
            ...
            if n_samples != X.shape[0]:
                raise ValueError("Number of samples in _X and doc_topic_distr"
                                 " do not match.")

            if n_topics != self.n_topics:
                raise ValueError("Number of _topics does not match.")

        doc_topic_distr = check_normalized(doc_topic_distr)
        self.components_ = check_normalized(self.components_)

        current_samples = X.shape[0]
        # bound = self._approx_bound(_X, doc_topic_distr, sub_sampling)
        bound = log_likelihood(X, doc_topic_distr, self.components_)
    ...
    def _n_parameters(self, threshold=EPS):
        K, V = self.components_.shape
        return K * (V - 1) + np.sum(self.doc_topic_distr > threshold)
